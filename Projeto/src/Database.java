	...
	*/
	...
	public synchronized void incrementDeg2(String fileHash, int chunkNo) {
		...
		for(int i = 0; i < chunksSaved.size(); i++){
			...
			if(chunksSaved.get(i).getFileId() == fileHash && chunksSaved.get(i).getChunkNo() == chunkNo){
	...
	public synchronized  String getFileHash(String filename){
		for(Map.Entry<String, String> entry : backedUpFiles2.entrySet()){
			String file = entry.getValue(); // ArrayList<Chunk> que tem chunks
				return entry.getKey();
			}
		return null;}
		*/
	//version 2
	/*
	public synchronized int getNrOfChunksSaved2(String fileHash) {
		return chunksSaved.size();
	}
	*/
